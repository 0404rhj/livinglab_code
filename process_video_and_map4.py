...
# 코드 안의 이 부분을 내일 이렇게 수정해 보세요!
ffmpeg_cmd = [
    'ffmpeg', '-y',
    '-i', LOCAL_AVI,
    '-c:v', 'libx264',
    '-preset', 'ultrafast',  # <-- 'fast' 대신 'ultrafast'로 변경 (속도 대폭 향상)
    '-crf', '24',            # <-- 용량 및 변환 부하 감소
    '-c:a', 'aac',
    '-b:a', '128k',
    '-movflags', '+faststart',
    LOCAL_MP4
...
# GCS URL 대신 mp4 파일을 직접 읽어서 base64 데이터로 HTML에 포함
print("영상을 HTML 내장 데이터로 인코딩 중...")
